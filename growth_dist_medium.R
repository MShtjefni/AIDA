...
random_sample_medium_firms<- growth_medium_firms[sample(nrow(growth_medium_firms), 500), ]

growth_random_medium <- as.numeric(random_sample_medium_firms$Growth[!is.na(random_sample_medium_firms$Growth)])

mean(growth_random_medium)
length(growth_random_medium)
shifted<- growth_random_medium +10 # to fit distributions that require values to be positive

scaled <- (growth_random_medium - min(growth_random_medium) +0.000001) /(max(growth_random_medium) - min(growth_random_medium)+ 0.000002)
#fit distributions that require values to be within ]0, 1[

LL2 <- function(m, s) { 
  -sum(dlaplace(growth_random_medium, m, s, log=TRUE)) 
}
...
descdist(growth_random_medium, discrete = FALSE)

#################################################################################################
#Fit distributions using fitdist and mle2
#norm, exp, gamma, beta, weibull, cauchy, pareto, logis, lnorm, laplace
fit_norm <- fitdist(growth_random_medium, distr="norm", method="mle")
fit_cauchy <- fitdist(growth_random_medium, distr="cauchy", method="mle")
fit_laplace<-mle2(LL2, start=list(m=1, s=0.001), method = "L-BFGS-B", lower = c(m=-0.1, s=0.001))
fit_logis <-fitdist(growth_random_medium, distr="logis", method="mle")
...
#KS test
ks.test(growth_random_medium, "pnorm", fit_norm$estimate[1], fit_norm$estimate[2])
ks.test(growth_random_medium, "pcauchy", fit_cauchy$estimate[1], fit_cauchy$estimate[2])
ks.test(growth_random_medium, "plaplace", coef(fit_laplace)[1] , coef(fit_laplace)[2])
ks.test(growth_random_medium, "plogis",fit_logis$estimate[1], fit_logis$estimate[2])
...
stats <- replicate(n.sims, {
  r <- rcauchy(length(growth_random_medium)
...
1-plogspline(ks.test(growth_random_medium
                     , "pcauchy"
                     , fit_cauchy$estimate[1]
                     , fit_cauchy$estimate[2])$statistic
             , fit_log
...
breaks <- -Inf
breaks <- append (breaks, c(seqlast(min(growth_random_medium),max(growth_random_medium),0.2), Inf))
growth_random_medium.cut<-cut(growth_random_medium,breaks=breaks, include.lowest = TRUE) #binning data
#table(growth_random_medium.cut) # frequencies of empirical data in the respective bins
p<-c()
...
for (j in 1:(length(breaks)-1)) {
  f.os[j]<- table(growth_random_medium.cut)[[j]]
}
...
x<-growth_random_medium #(normal, cauchy, laplace, logis)
x<-shifted #(lnorm, pareto, exp, weib, gamma)
x<-scaled #(beta)

par(mfrow=c(1,1))

hist(x, prob=T, breaks="fd", xlab="growth_random_medium rate", col="grey", main="Empirical small growth Rate Distribution")
...
#For other objects:
#QQ plot
lengr <- length(growth_random_medium)
y <- rlaplace(lengr, coef(fit_laplace)[1], coef(fit_laplace)[2])
qqplot(y, growth_random_medium, xlab="Theoretical Quantiles", ylab = "Empirical Quantiles")
...
plot(ecdf(growth_random_medium),cex=0.5, col="red")
