...
growth_manufacturing_north<-subset(growth_manufacturing, growth_manufacturing$GeoArea == "Nord")
growth_manufacturing_center<-subset(growth_manufacturing, growth_manufacturing$GeoArea == "Centro")
growth_manufacturing_south<-subset(growth_manufacturing, growth_manufacturing$GeoArea == "Sud")

random_sample_north<- growth_manufacturing_north[sample(nrow(growth_manufacturing_north), 500), ]
random_sample_center<- growth_manufacturing_center[sample(nrow(growth_manufacturing_center), 500), ]
random_sample_south<- growth_manufacturing_south[sample(nrow(growth_manufacturing_south), 500), ]

growth_random_north <- as.numeric(random_sample_north$Growth[!is.na(random_sample_north$Growth)])
growth_random_center <- as.numeric(random_sample_center$Growth[!is.na(random_sample_center$Growth)])
growth_random_south <- as.numeric(random_sample_south$Growth[!is.na(random_sample_south$Growth)])


mean(growth_random_center)
length(growth_random_center)
shifted<- growth_random_center +10 # to fit distributions that require values to be positive

scaled <- (growth_random_center - min(growth_random_center) +0.000001) /(max(growth_random_center) - min(growth_random_center)+ 0.000002)
#fit distributions that require values to be within ]0, 1[

LL2 <- function(m, s) { 
  -sum(dlaplace(growth_random_center, m, s, log=TRUE)) 
}
...
descdist(growth_random_center, discrete = FALSE)

#################################################################################################
#Fit distributions using fitdist and mle2
#norm, exp, gamma, beta, weibull, cauchy, pareto, logis, lnorm, laplace
fit_norm <- fitdist(growth_random_center, distr="norm", method="mle")
fit_cauchy <- fitdist(growth_random_center, distr="cauchy", method="mle")
fit_laplace<-mle2(LL2, start=list(m=1, s=0.001), method = "L-BFGS-B", lower = c(m=-0.1, s=0.001))
fit_logis <-fitdist(growth_random_center, distr="logis", method="mle")
...
#KS test
ks.test(growth_random_center, "pnorm", fit_norm$estimate[1], fit_norm$estimate[2])
ks.test(growth_random_center, "pcauchy", fit_cauchy$estimate[1], fit_cauchy$estimate[2])
ks.test(growth_random_center, "plaplace", coef(fit_laplace)[1] , coef(fit_laplace)[2])
ks.test(growth_random_center, "plogis",fit_logis$estimate[1], fit_logis$estimate[2])
...
stats <- replicate(n.sims, {
  r <- rcauchy(length(growth_random_center)
...
1-plogspline(ks.test(growth_random_center
                     , "pcauchy"
                     , fit_cauchy$estimate[1]
                     , fit_cauchy$estimate[2])$statistic
             , fit_log
...
breaks <- -Inf
breaks <- append (breaks, c(seqlast(min(growth_random_center),max(growth_random_center),0.2), Inf))
growth_random_center.cut<-cut(growth_random_center,breaks=breaks, include.lowest = TRUE) #binning data
#table(growth_random_center.cut) # frequencies of empirical data in the respective bins
p<-c()
...
for (j in 1:(length(breaks)-1)) {
  f.os[j]<- table(growth_random_center.cut)[[j]]
}
...
x<-growth_random_center #(normal, cauchy, laplace, logis)
x<-shifted #(lnorm, pareto, exp, weib, gamma)
x<-scaled #(beta)

par(mfrow=c(1,1))

hist(x, prob=T, breaks="fd", xlab="growth_random_center rate",xlim = c(-3, 4), col="grey", main="Empirical small growth Rate Distribution")
...
#For other objects:
#QQ plot
lengr <- length(growth_random_center)
y <- rlaplace(lengr, coef(fit_laplace)[1], coef(fit_laplace)[2])
qqplot(y, growth_random_center, xlab="Theoretical Quantiles", ylab = "Empirical Quantiles")
...
plot(ecdf(growth_random_center),cex=0.5, col="red")
