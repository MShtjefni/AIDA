...
random_sample_small_firms<- growth_small_firms[sample(nrow(growth_small_firms), 500), ]

growth_random_small <- as.numeric(random_sample_small_firms$Growth[!is.na(random_sample_small_firms$Growth)])

mean(growth_random_small)
length(growth_random_small)
shifted<- growth_random_small +10 # to fit distributions that require values to be positive

scaled <- (growth_random_small - min(growth_random_small) +0.000001) /(max(growth_random_small) - min(growth_random_small)+ 0.000002)
#fit distributions that require values to be within ]0, 1[

LL2 <- function(m, s) { 
  -sum(dlaplace(growth_random_small, m, s, log=TRUE)) 
}
...
descdist(growth_random_small, discrete = FALSE)

#################################################################################################
#Fit distributions using fitdist and mle2
#norm, exp, gamma, beta, weibull, cauchy, pareto, logis, lnorm, laplace
fit_norm <- fitdist(growth_random_small, distr="norm", method="mle")
fit_cauchy <- fitdist(growth_random_small, distr="cauchy", method="mle")
fit_laplace<-mle2(LL2, start=list(m=1, s=0.001), method = "L-BFGS-B", lower = c(m=-0.1, s=0.001))
fit_logis <-fitdist(growth_random_small, distr="logis", method="mle")
...
#KS test
ks.test(growth_random_small, "pnorm", fit_norm$estimate[1], fit_norm$estimate[2])
ks.test(growth_random_small, "pcauchy", fit_cauchy$estimate[1], fit_cauchy$estimate[2])
ks.test(growth_random_small, "plaplace", coef(fit_laplace)[1] , coef(fit_laplace)[2])
ks.test(growth_random_small, "plogis",fit_logis$estimate[1], fit_logis$estimate[2])
...
stats <- replicate(n.sims, {
  r <- rcauchy(length(growth_random_small)
...
1-plogspline(ks.test(growth_random_small
                     , "pcauchy"
                     , fit_cauchy$estimate[1]
                     , fit_cauchy$estimate[2])$statistic
             , fit_log
...
breaks <- -Inf
breaks <- append (breaks, c(seqlast(min(growth_random_small),max(growth_random_small),0.2), Inf))
growth_random_small.cut<-cut(growth_random_small,breaks=breaks, include.lowest = TRUE) #binning data
#table(growth_random_small.cut) # frequencies of empirical data in the respective bins
p<-c()
...
for (j in 1:(length(breaks)-1)) {
  f.os[j]<- table(growth_random_small.cut)[[j]]
}
...
x<-growth_random_small #(normal, cauchy, laplace, logis)
x<-shifted #(lnorm, pareto, exp, weib, gamma)
x<-scaled #(beta)

par(mfrow=c(1,1))

hist(x, prob=T, breaks="fd", xlab="growth_random_small rate",xlim = c(-3, 4), col="grey", main="Empirical small growth Rate Distribution")
...
#For other objects:
#QQ plot
lengr <- length(growth_random_small)
y <- rlaplace(lengr, coef(fit_laplace)[1], coef(fit_laplace)[2])
qqplot(y, growth_random_small, xlab="Theoretical Quantiles", ylab = "Empirical Quantiles")
...
plot(ecdf(growth_random_small),cex=0.5, col="red")
